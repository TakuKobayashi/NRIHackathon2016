...
public class FericaRecord {
    ...
    public static FericaRecord parse(byte[] res, int off) {
        FericaRecord self = new FericaRecord();
        self.termId = res[off+0]; //0: 端末種
        self.procId = res[off+1]; //1: 処理
        //2-3: ??
        int mixInt = self.toInt(res, off, 4,5);
        self.year  = (mixInt >> 9) & 0x07f;
        self.month = (mixInt >> 5) & 0x00f;
        self.day   = mixInt & 0x01f;

        if (self.isBuppan(self.procId)) {
            self.kind = "物販";
        } else if (self.isBus(self.procId)) {
            self.kind = "バス";
        } else {
            self.kind = res[off+6] < 0x80 ? "JR" : "公営/私鉄" ;
        }
        self.remain  = self.toInt(res, off, 11,10); //10-11: 残高 (little endian)
        self.seqNo   = self.toInt(res, off, 12,13,14); //12-14: 連番
        self.reasion = res[off+15]; //15: リージョン
        return self;
    }
